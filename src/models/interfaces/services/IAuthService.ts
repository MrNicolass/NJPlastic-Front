...
/**
 * Contract of every authentication-bound REST call delivered by the
 * backend and consumed by the frontend. Mirrors the project service
 * convention: every method accepts an optional `suppressError` flag as
 * the last parameter; when true the global Axios interceptor skips the
 * error notification so the caller can render its own feedback.
 */
