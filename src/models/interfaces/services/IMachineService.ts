...
/**
 * Contract of every machine-bound REST call delivered by EP-BE-05. All
 * methods accept an optional {@link suppressError} flag as the last
...
export interface IMachineService {
  ...
  /**
   * Edits the message of an AUTO_STOPPED record (UC12, RF18, RF19).
   *
  ...
  /**
   * Calculates the OEE of a machine over the requested window (RF10).
   *
   * @param machineId - The machine UUID.
   * @param from - Window start in ISO 8601 with offset.
   * @param to - Window end in ISO 8601 with offset.
  ...
  /**
   * Registers quality counts that close the OEE quality factor (RF10).
   *
   * @param machineId - The machine UUID.
  ...
  /**
   * Generates the consolidated shift report (RF15).
   *
   * @param from - Window start in ISO 8601 with offset.
   * @param to - Window end in ISO 8601 with offset.
